...
function _domReady(f){
	...
	/in/.test(document.readyState)?setTimeout(function(){_domReady(f);},9):f();
};
...
function _getPath(){
	...
	for(var i=0; i<scripts.length; i++){
		...
		if (!src) continue;
		var m = src.match(/(\/syntaxhighlighter\.js(\W|$))|(\"syntaxhighlighter\.js(\W|$))|(\'syntaxhighlighter\.js(\W|$))/i);
		if (m){
			_getPath.full = src;
...
var _path = _getPath();
var _theme = (_getPath.full || "").split("?"),
	_theme = _theme.length == 2 ? _theme[1] : "default",
	_theme = {
		django:"Django", 
		eclipse:"Eclipse", 
		emacs:"Emacs", 
		...
		midnight:"Midnight", 
		rdark:"RDark"
	}[_theme.toLowerCase()] || "Default";

_loadCss(_path + "styles/shCore.css?ver=2.1.364");
_loadCss(_path + "styles/shTheme" + _theme + ".css?ver=2.1.364");


SyntaxHighlighter.config.clipboardSwf = _path + 'clipboard.swf';
