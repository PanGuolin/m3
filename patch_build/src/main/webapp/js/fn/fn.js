...
(function($){
	...
	var STRING = {
		...
		/** 将对象转换成字符串 
		 ...
		 @param {String} [indentChar="    "] 缩进符
		 @param {String} [indent] 缩进符长度
		*/
		json: function(obj, lineChar, indentChar, indent){
			var str = '';
			...
			// Object / Array / Boolean / Function / Number / String
			switch(obj.constructor){
				...
				case Object:
					for(var n in obj){
						...
						str += lineChar + indent + '    ';
						str += '"' + n + '": ';
						str += fn.objStr(obj[n], lineChar, indentChar, indent + indentChar);
					};
					return str ? ('{' + str + lineChar + indent + '}') : "{}";
			};
		...
		/** 
		 全部替换
		 @param {String} text 要执行替换的文本
		 @param {String} s1 被替换的文本
		 @param {String} s2 替换成的文本
		 @returns {String} 返回经过替换的字符串
		*/
		replace: function(text, s1, s2){
		...
		*/
		...
		/**  
		 字符串格式化
		 @param {String} format 格式化模板，以{n}作为占位符，以第2-n个参数替代占位符的值
		 @param {String} arg0 替代占位符的值， 此参数数量可变
		 @param {String} arg1 替代占位符的值， 此参数可选
		 @param {String} argN 替代占位符的值， 此参数可选
		 @example var str = fn.format("{0}月{1}日", d.getYear(), d.getMonth() + 1);
		*/
		format: function(){
		    var param = [];
		...
		/** 
		 把 aa=1&bb=2 格式的字符串转换成 { aa: "1", bb: "2" } 格式的对象
		*/
		...
		*/
		...
		/** 
		 读取dom元素的单个属性并自动转换其数据类型并将其值返回
		 @param {String|dom|jquery} element 要读取的dom对象
	...
	var _isInit = false,
		FN_PATH = "js/fn/", // 文件夹相对于web服务器根目录路径
		FN_PATHS = ["js/fn/1.5/", "lib/fn-1.3.1/"],
		FN_FILENAME = "fn.js", // 文件名称
	...
	/** @lends fn */
	{
		...
		*/
		...
		/** 查询元素elem在数组array中的索引号
		 @function
		 @returns {integer} index 元素elem在数组array中的索引号
		*/
		...
		/** @private */
		tagFunc: function(func){
			
			if(!func) return null;
			
			func = fn.trim(func.toString());
			
			// 兼容万恶的IE8
			var funcS = fn.replace(func, " ", "");
			var regex = /\{[^}]*\}/g;
			var ret = funcS.match(regex);
			if(ret && ret.length == 1){
				var name = ret[0].replace(/[^(a-zA-z|\d)]/g, "");
				if(fn.isFunction(window[name])) return window[name];
			};
			
			// 兼容万恶的IE
			var s = "function anonymous()\n{";
			
			if(func.indexOf(s) == 0 && func.substr(func.length-1) == "}"){
				func = fn.trim(func.substr(s.length, func.length - s.length - 1));
			};
			...
			if(window[func]){
				...
				//
				func = 'window["tagFunc_tmp"] = ' + func + ';';
				( window.execScript || function( data ) {
					eval.call( window, data );
				} )( func );
				func = window["tagFunc_tmp"];
				
			}else{
				//
				func = 'window["tagFunc_tmp"] = function(){ ' + func + '; };';
				( window.execScript || function( data ) {
					eval.call( window, data );
				} )( func );
				func = window["tagFunc_tmp"];
			...
			return fn.isFunction(func) ? func : null;
		},
		...
		/** 
		 ...
		 @param {String} attrName 属性名
		*/
		tagAttr: function(el, attrName, ns){
			...
			// 数字
			}else if(attr && !isNaN(attr)){ 
				return attr.indexOf(".") > -1 ?
					parseFloat(attr) : parseInt(attr);
					
			// 函数
			}else if(
				attrName.toLowerCase() == "onselect" ||
				attrName.toLowerCase() == "onchange" ||
				attrName.toLowerCase() == "oncheck" ||
				attrName.toLowerCase() == "onclick" ||
				attrName.toLowerCase() == "ondbclick" ||
				attrName.toLowerCase() == "jsonhandler"
				){
				return fn.tagFunc(attr);
				
			// 变量或函数
			}else if(
				attrName.toLowerCase().indexOf("on") == 0 ||
				attr.indexOf("function") == 0 ||
				attr.indexOf("[") > -1 || 
				attr.indexOf("{") > -1 || 
				attr.indexOf(".") > -1 || 
				attr.indexOf("(") > -1
				...
				try{
					if(attr.indexOf("function") == 0){
						
					}else{
						// 如果最后一个字符是")"或者";"
						var lastChar = attr.charAt(attr.length - 1);
						if(lastChar == ")" || lastChar == ";"){
							attr = "function(){" + attr + ";}";
						};
					};
					eval("window.__temp = " + attr + ";");
					return window.__temp;
				}catch(ex){
					return attr;
				}
		...
		/** 
		 读取dom元素的多个属性并自动转换其数据类型， 返回一个简单{}对象
		 @param {String|dom|jquery} element 要读取的dom对象
		 @param {String} attrNames 属性名，多个用逗号分隔，例如： "max, min, lang, format"
		*/
		tagAttrs: function(el, attrs){
			if($){
				el = $(el).get(0);
			};
			if(!el) return {};
			...
			for(var i = 0, len = attrs.length; i < len; i++){
				var attr = fn.trim(attrs[i]);
				
				var sp = attr.split(":");
				var real = sp.length > 1 ? fn.trim(sp[1]) : attr;
				attr = fn.trim(sp[0]);
				
				if(real && (el.attributes[real] || ($ ? $(el).attr(real) : el.getAttribute(real)))){
					obj[attr] = fn.tagAttr(el, real);
				};
			};
		...
		*/
		form2obj: function(form){
			...
			if(form.length){
				...
				for(var n in names){
					var vals = [];
					$.each(names[n], function(i, input){
						var val;
						if(input.type === "checkbox" || input.type === "radio"){
							if(input.checked) vals.push($(input).val());
						}else{
							vals.push($(input).val());
						};
					});
					params.push(n + '=' + vals.join(','));
				};
			};
			return fn.string.url2obj(params.join("&"));
		},
		
		/*
		 将表单进行序列化成url
		*/
		form2param: function(form, encode){
			form = $(form);
			if(!form.length){
				return "";
			}else{
		...
		/*
		 拷贝对象
		 可以接受n个参数， 依次把所有对象的属性或者参数覆盖或者追加到第一个参数上
		 如果第一个参数是object， 则为覆盖属性
		 如果第一个参数是字符串， 则为url参数的追加
		 例如： 
		 fn.paramExtend("a=1", {a: 2})  的结果为 "a=1&a=2"
		 fn.paramExtend({a: 2}, "a=1")  的结果为 {a: 1}
		 fn.paramExtend("a=1", {a: [2, 3]})  的结果为 "a=1&a=2&a=3"
		 fn.paramExtend({a: 4}, "a=1", {a: [2, 3]})  的结果为 "a=2&a=3"
		*/
		paramExtend: function(des){
			var args = arguments;
			for(var i = 1; i < args.length; i++){
				des = _extend(des, args[i]);
			};
			// 
			function _extend(des, src){
				...
				}else if($.isPlainObject(des)){
					if(typeof src === "string"){
						src = fn.unparam(src);
					};
					return $.extend(des, src);
				};
			};
			return des;
		},
		...
		/*
		 把对象转换成URL；
		 例如： {a:[1,2]} 会转换成 a=1&a=2
		*/
		param: function(obj, encode){
			if(typeof obj !== "object"){
				return obj;
			};
			
			var obj = $.param(obj);
			obj = fn.replace(obj, "%5B%5D=", "=");
			if(!encode){
		...
		*/
		...
		/** 根据 name 删除 dom */
		removeTagsByName: function(/*String*/ name){
		...
		/** 加载CSS */
		loadCss: function(/*String[], String*/ href, /*String?*/ name){
			
			function _loadCss(href, name){
				href = fn._formatUrl(href);
				
				var tag = document.createElement("link");
				if(name){
					//tag.setAttribute("name", name);
					//fn.removeTagsByName(name);
				};
		...
		/** 绑定文档加载完成后执行函数 */
		domReady: function(f){
			if(!fn.isFunction(f)) return;
			/in/.test(document.readyState)?setTimeout(function(){fn.domReady(f);},9):f();
		}
	};
	...
	fn.getPath = function(){
		...
		for(var i=0; i<scripts.length; i++){
			...
			if (!src) continue;
			var m = src.match(/(\/fn\.js(\W|$))|(\"fn\.js(\W|$))|(\'fn\.js(\W|$))/i);
			if (m){
				base = src.substring(0, m.index + 1);
				
				fn._script = scripts[i];
				fn.fullPath = src;
				fn.path = base;
			};
		};
	...
	fn.path = fn.getPath(); // 根路径
	// 先取host地址
	fn.hostPath = location.protocol + "//" + location.host + "/";
	// 再尝试
	for(var i = 0; i < FN_PATHS.length; i++){
		if(fn.path.indexOf(FN_PATHS[i]) > -1){
			// 假设库的存放路径为FN_PATHS[i]
			fn.hostPath = fn.path.replace(FN_PATHS[i], "");
			break;
		};
	};
	
	/*
	 某些情况下，会出现fn.money.js之类的独立文件被放在fn.js之前引用， 所以判断如果fn.version定义不存在， 则此fn会被重新定义
	*/
	if(window.fn && window.fn.version){
		window.fn = window.fn;
	}else{
		window.fn = fn;
	};
	...
	*/
	...
	/**
	 格式化URL， 将{root}/替换成根路径， {fn}/替换成fn目录路径， ./替换成当前路径
	 @private
	 @param {String} src 要替换的url地址
	*/
	fn._formatUrl = function(url){
		url = fn.trim(url);
			
		if(url.indexOf("{root}/") == 0){ // 站点根目录
			url = url.replace("{root}/", fn.hostPath);
			
		}else if(url.indexOf("{fn}/") == 0){ // fn根目录
			url = url.replace("{fn}/", fn.path);
			
		}else if(url.indexOf("http://") == 0){ // 完整路径不处理
			// do nothing
			
		//}else if(url.indexOf("../") == 0){ // 上一级目录
		//	url = fn.hostPath + url;
			
		}else if(url.indexOf("./") == 0){ // 相对于当前页面的路径
			url = url.replace("./", location.href.substring(0, location.href.lastIndexOf("/")) + "/");
			
		}else if(url.indexOf("/") == 0){ // 站点根目录，不处理
	...
	fn.url = {
		format: fn._formatUrl
	};
	
	
	/** 
	 动态载入js文件
	 @param {String|String[]} src 加载的js地址，可为数组
	 @param {Object}    opt                      可选参数集合
	 @param {String}    opt.name                 标签名
	 @param {Function}  opt.success              所有js加载成功后回调函数
	 @param {Function}  opt.error                有js加载失败时的回调函数
	 @param {Function}  opt.complete             不管是否加载成功， 都会调用的回调函数
	 @param {Function}  [opt.timeout = 5000]     超时时间
	 @param {Boolean}   [opt.multiple = false]   是否并行加载，默认为false
	 @param {Boolean}   [opt.async = false]      是否异步加载（不阻塞进程），默认为false
	*/
	fn.loadScript = function(src, opt){
		...
		opt = $.extend({
			...
			multiple: false,
			async: false,
			type: fn.loadScript.type || "ajax"
		}, opt);
		
		
		// 加载单个脚本
		function _loadScriptTag(src, callback){
			if(!src) return;
			...
			if(tag.readyState) { //for IE
				tag.onreadystatechange = function() {
					if(tag.readyState == "loaded" || tag.readyState == "complete") {
						tag.onreadystatechange = null; //此处销毁事件引用，防止IE下事件驻留内存
			...
			}; // end if if(tag.readyState) {
			
			head.appendChild(tag);
		}; // end function loadScript(src, name, callback){
		
		// 加载单个脚本
		function _loadScript(src, success){
			if(!src) return;
			
			src = fn._formatUrl(src);
			
			// 默认以ajax方式加载js
			if(opt.type == "tag"){
				_loadScriptTag(src, success)
			}else{
				$.ajax({
					url: src,
					dataType: "script",
		...
		}; // end function loadScript(src, name, success){
		
		// 如果是传入单个脚本地址，直接调用loadScript，否则...
		if(fn.isArray(src)){
			//
			if(!src.length){
			...
			//
			if(!opt.multiple || !opt.async){
				function loadJs(index){
					_loadScript(src[index], index == src.length - 1 ? opt.success : function(){
						loadJs(index + 1);
					});
				};
				loadJs(0);
				...
				function cb(){
					...
					if(count == 0){
						if(opt.success) opt.success();
						if(opt.complate) opt.complate();
					};
				};
				// 该函数与cb的区别在于，3000毫秒之后如果未能加载完所有Script，直接触发success
				function lastCb(){
					cb();
					setTimeout(function(){
						if(count == 0){
				...
				for(var i = 0; i < src.length; i++){
					// 加载最后一个Script时传入lastCb函数
					var isLast = i == src.length - 1;
					//_loadScript(src[i], opt.isLast ? lastCb : cb);
					_loadScript(src[i], isLast ? lastCb : cb);
				};
			};
		}else{
			_loadScript(src, opt.success);
		}; // end if(isArray(src)){
	}; // end loadScript
	...
	/** 注册模块 
	 @param {Object}        opt
	 @prama {String}        opt.name        模块名称
	 @prama {String}        opt.namespace   模块命名空间
	 @prama {String}        opt.methods     模块内方法名， 以逗号分隔
	 @prama {String}        opt.rely        依赖的模块名称，多个用逗号分隔
	 @prama {Function}      opt.check       检查模块是否已经加载过的函数，返回true或false
	 @prama {[String]}      opt.script      模块包含的script地址数组
	 @prama {[String]}      opt.css         模块包含的css地址数组
	 @prama {Function}      opt.onLoad      模块加载后的回调函数
	 @prama {Function}      opt.beforeLoad  模块加载前的回调函数
	 @prama {Function}      opt.success     模块加载成功回调函数
	 @prama {Boolean}       opt.multiple    是否并发加载js，但当加载方式为同步（async=true）时并发会失效
	 @param {Number|String} opt.delay       模块默认延迟加载时间，以毫秒表示，如果是"domReady"表示延迟到页面加载完成后
	*/
	fn.reg = function(opt){
		...
		if(reg.rely.length) reg.rely = fn.replace(reg.rely, " ", "").split(",");
		
		// 检查
		if(!reg.name ||
			!fn.isFunction(reg.check) ||
			!fn.isArray(reg.script) ||
			!fn.isArray(reg.css) 
		) return false;
		
		// 以多个名称注册
		var names = reg.name.split(","),
			name;
		for(var i = 0; i < names.length; i++){
			name = fn.trim(names[i]);
			fn._regs[name] = reg;
		}; // end for
		
		// 注册方法
		if(reg.methods && name){
			fn.regMethod(reg.methods, name, reg.namespace);
		};
	...
	*/
	...
	fn.load = function(paths, opt){
		...
		// 延迟加载
		if(typeof opt == "number") opt = {delay: opt};
		// 回调函数
		...
		for(var i = 0; i < paths.length; i++){
			...
			path = path.replace(/^{fn}\//g, ""); // 为防止{fn}出现两次， 先删除
			path = path.split("/");
			
			var name = path[path.length - 1],   // 模块名（最后一段）
				path = path.splice(0, path.length - 1).join("/"), // 中间路径部分
				url = [];
			...
			// 如果不以.js结尾， 视为文件夹
			if(name.indexOf(".js") > -1 && name.indexOf(".js") === name.length - 3){
				url.push(name);
			}else{
				url.push(name);
				url.push(name + ".js");
		...
		fn.loadScript(scripts, {
			success: function(){
				// 用户的回调函数
				if(opt.success){
					opt.success.call(reg, $);
				};
			},
			async: opt.async,
	...
	/**
	 动态加载模块
	 @param {String} module 加载的模块名称，如多个以逗号分隔
	 @param {Number|String|Function|Object} opt          参数设置，如是Number型表示opt.delay，如是Function型表示opt.success
	 @param {Number|String}                 opt.delay    模块加载前的延时（单位毫秒），
	 	如不设置（默认等于0）表示立刻加载js文件， 方式为同步加载（阻塞浏览器，直至加载完成），
	 	如该值大于0， 则在相应的毫秒数后加载js文件， 方式为异步加载（不阻塞浏览器，继续执行后续代码），
	 	如该值为"domReady"， 则在页面<body>标签闭合后开始加载， 方式为异步加载（不阻塞浏览器，继续执行后续代码）
	 @param {Function}                      opt.success  模块加载完成后的回调函数， 该参数会接受一个参数：window.jQuery
	 @param {Boolean}                       opt.async    是否异步加载（不阻塞进程），默认为false
	*/
	fn.use = function(names, opt){
		if(!names) return;
		
		names = fn.replace(names, " ", "").split(",");
		
		var args = arguments, i = 0, reg0 = fn._regs[names[0]];
		...
		// 等待文档加载完成后加载/延迟加载/立刻加载
		if(opt.delay == "domReady"){
			fn.domReady(start);
		}else if(typeof opt.delay == "number" && opt.delay > 0){
			setTimeout(start, opt.delay);
		}else{
			start();
		};
		...
		function start(){
			if(reg0){
				...
				fn.load.apply(this, args);
			};
		};
		
		// 加载第i个模块
		function loadModel(){
			var reg = fn._regs[names[i]];
			
			// 如果当前模块并未在注册列表中找到，跳过
			if(!reg){
				if(names[i]){
					fn.log("没有找到模块'" + names[i] + "'");
				};
				if(i < fn._regs.length - 1){
					i++;
					loadModel();
				};
				return;
			...
			// 加载依赖模块
			// 加载最后一个依赖模块之后调用doLoadModel加载当前模块
			function loadRely(x){
				
				var rely = fn._regs[reg.rely[x]];
				
				if(!rely && x < reg.rely.length - 1){
					// 如果依赖模块并未在注册列表中找到，跳过
					loadRely(x+1);
					return;
				}else if(x < reg.rely.length - 1){
					// 加载完当前依赖模块后加载下一个依赖模块
					// 加载完最后依赖模块后调用doLoadModel加载当前模块
					doLoadRely(rely, function(){
						loadRely(x+1);
					});
				}else{
					// 最后一个
					doLoadRely(rely, doLoadModel);
				};
				
				// 调用fn.use加载依赖模块
				// 如果模块已经加载过了，直接调用回调函数
				function doLoadRely(rely, relyCb){
					if(reg.loaded || (reg.check && reg.check()) || !rely){
						relyCb();
					}else{
						// 加载下一个模块（同步阻塞式加载）
						fn.use(rely.name, relyCb);
					}
				}; // end doLoadRely
			}; // end loadRely
			...
			function doLoadModel(){
				if(reg.loaded || reg.check()){
					next();
				}else{
					if(fn.isFunction(reg.beforeLoad)) reg.beforeLoad();
					fn.loadCss(reg.css, reg.name);
					fn.loadScript(reg.script, {
						name: reg.name, 
						success: function(){
							reg.loaded = true;
							if(fn.isFunction(reg.onLoad)) reg.onLoad();
							next();
						},
						async: opt.async,
				...
				function next(){
					if(i == names.length - 1){
						(function(){
							// 模块的回调函数
							if(reg.success){
								if(reg.success.call(reg, opt.success) == false){
									return;
								};
							};
							// 用户的回调函数
	...
	/** 
	 注册虚函数（函数被调用时才从外部引入js）
	 @param {String} methods   所要注册之方法名， 以逗号分隔
	 @param {String} module    方法所需之模块名
	 @param {String} namespace 方法所在的命名空间，省略此参数则命名空间为window
	*/
	fn.regMethod = function(methods, module, namespace){
		
		// 遍历以逗号分隔的funcName
		methods = methods.split ? methods.split(",") : methods || [];
		
		// 逐个注册
		for(var i = 0; i < methods.length; i++){
			// 去空格
			var method = $.trim(methods[i]);
			if(method){ 
				doReg(method);
			};
		}; // end for
		...
		// 注册单个
		function doReg(method){
			
			// 如果方法名以.开头， 冠之以namespace
			if(method.charAt(0) == "."){
				method = namespace + method;
			};
			
			// 创建虚方法
			fn._vMethods[method] = fn.ns(method, function(){
				// 载入依赖模块
				fn.use(module, { async: false, delay: 0});
				
				// 如果虚函数还是虚的， 说明js加载失败了
				if(fn._vMethods[method] == fn.ns(method)){
					var msg = '位于[' + module + ']模块的[' + method + ']函数注册失败';
					
					// 将虚函数置空
					fn.ns(method, function(){
						fn.ns(method, function(){});
						try{
							// 记录错误
							window.console && console.log(msg);
				...
				// 返回计算结果
				return fn.ns(method).apply(this, arguments);
			});
		}; // end doReg
	...
	/**
	 获取dom对象
	 @param {String|dom|jquery} selector 选择器
	*/
	fn.get = function(selector){
		var obj, s = selector;
		
		obj = document.getElementById(s) || $(s);
	...
	*/
	...
	// 获取<script>标签内参数
	fn._param = (function(){
		var p = (fn.fullPath || "").split("?");
		
		if(p.length == 1){
			p.push("");
		};
		
		p = fn.urlStrToObj(p[1]);
		...
		if(!p.use){
			...
			var mods = p.use.split(",");
			p.use = [];
			
			// 取得加载模块名及参数列表
			for(var i = 0; i < mods.length; i++){
				var mod = mods[i].replace("]","").split("["),
					...
					cfg = (function(str){
						...
						// 把delay:2000|async:true格式的参数转换成Object
						var c = str.split("|");
						...
						// 数据类型转换
						if(!isNaN(ret.delay)) ret.delay = parseInt(ret.delay);
						if(ret.async) ret.async = ret.async == "true";
						if(ret.success) ret.success = fn.ns(ret.success);
	...
	if(location.href.indexOf("file:///") == 0){
		fn.loadScript.type = "tag";
		fn.ignoreError = true;
		// 加载静态demo辅助显示模块
		fn.loadScript("{fn}/lib/widget/1.3/demo/demo.js");
	};
	
	// 自动加载相应版本的配置文件
	fn.loadScript("{fn}/configs/" + fn._param.v + ".js", {
		async: false,
		success: function(){
			// 加载url中指定模块
			var p = fn._param || {};
			if(p.use && p.use.length){
				for(var i = 0; i < p.use.length; i++){
					var u = p.use[i];
					fn.use(u.name, u.opt);
				};
			};
