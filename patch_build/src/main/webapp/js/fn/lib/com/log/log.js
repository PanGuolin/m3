...
(function($) {
	...
	function generateMarkup() { //build markup
		...
		newNode.innerHTML = [
			...
			'<div class="footer">',
				'<div class="left">',
					//'<label for="', IDs.checkbox, '"><input type="checkbox" id="', IDs.checkbox, '" />Visible on page load</label>',
					'<label><input type="text" class="console" style="width:200px; "/><span style="color:#FFF;padding: 2px 0 0 5px; position: absolute; width: 50px; ">回车运行</span></label>',
				'</div>',
				'<div class="right">',
				'</div>',
			'</div>'
	...
	log = {
		toggle:
			function() { $(isVisible() ? hide : show);  return log;},
		show: 
			function(){ $(show); return log;},
		hide: 
			function(){ $(hide);  return log;},
		resize:
			function() { $(resize);  return log;},
		clear:
			function() { $(clear);  return log;},
		move:
			function() { $(reposition);  return log;},
		debug: 
			function( msg ) { addMessage( 'debug', msg );  return log;},
		warn:
			function( msg ) { addMessage( 'warn', msg );  return log;},
		info:
			function( msg ) { addMessage( 'info', msg );  return log;},
		error:
			function( msg ) { addMessage( 'error', msg );  return log;},
		profile: 
			function( label ) {
	...
	$(function() {
		...
		$(".console", bbird).keypress(function(e){
			if(e.keyCode == 13){
				try{
					window.___temp = undefined;
					var val = ("_X_X_X_" + $.trim(this.value)).replace("_X_X_X_=", "").replace("_X_X_X_", ""),
						show = this.value.charAt(0) == "=";
					
					try{
						fn.globalEval("window.___temp = " + val);
					}catch(e){
						fn.globalEval(val);
					};
					
					if(show) log.debug(window.___temp + "");
				}catch(e){
					log.error(e);
				};
				this.value = "";
		...
		log.init = function() {
			show();
			log.error( [ '<b>', NAMESPACE, '</b> can only be initialized once' ] );
		}
